...
const Menu = ({ menu, open, setOpen }: { menu: any; open: any; setOpen: any }) => {
    ...
    >
      ...
      <AccordionDetails sx={{ padding: '0px' }}>
        <ul className="space-y-3 font-medium ml-10">
          {isLoading ? (
            <p className="h-4 w-full rounded-md"></p>
          ) : (
            data?.map((i: any, idx: number) => (
              <li
                ...
                key={idx}
              >
                <Link className="" key={Math.random()} href={`/blog/?menu=${menu}&c_id=${i.id}`}>
                  {i.title}
                </Link>
              </li>
            ))
          ...
          <div className="space-y-3 px-3 pt-7">
            ...
            {['স্থাপত্য', 'প্রকৌশল', 'ইন্টেরিয়র', 'পরিবেশ', 'মূল রচনা', 'অন্যান্য'].map(
              (i: any) => {
                return (
                  <div key={Math.random()}>
                    <Menu menu={i} open={open} setOpen={setOpen} />
                  </div>
                );
              },
            )}
